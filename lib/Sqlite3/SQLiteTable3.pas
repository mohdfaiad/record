...
type
  ...
  public
    ...
    property Locked: boolean read GetDbLocked;
...
constructor TSQLiteDatabase.Create(const FileName: string; const Key: string);
var
  Msg: PAnsiChar;
  iResult: integer;
  utf8FileName: UTF8string;
begin
  ...
  try
    utf8FileName := AnsiToUtf8(FileName);
    iResult := SQLite3_Open(PAnsiChar(utf8FileName), Fdb);
    SQLite3_Key(Fdb, PAnsiChar(Key), Length(Key));
    if iResult <> SQLITE_OK then
      if Assigned(Fdb) then
      begin
        Msg := Sqlite3_ErrMsg(Fdb);
...
function TSQLiteDatabase.GetDbLocked: boolean;
var
  Stmt: TSQLiteStmt;
  NextSQLStatement: PAnsiChar;
  r: Integer;
begin
  r := Sqlite3_Prepare(self.fDB, PAnsiChar('select * from sqlite_master'), -1, Stmt, NextSQLStatement);
  if r = SQLITE_OK then
  begin
    Result := False;
  end
  else if r = SQLITE_BUSY then
  begin
    Result := True;
  end;
...
begin
  ...
  begin
    case Bindings[I].VType of
      vtString, vtAnsiString, vtPChar, vtWideString, vtPWideChar, vtChar, vtWideChar:
        begin
          case Bindings[I].VType of
            vtString:
              begin // ShortString
          ...
          begin
            ...
            //get data types
              ...
              begin
                ...
                else if (AnsiStrUpper(DeclaredColType) = 'INTEGER') or (AnsiStrUpper(DeclaredColType) = 'BOOLEAN') or (AnsiStrUpper(DeclaredColType) = 'BIT') then
                  thisColType^ := dtInt
                else if (AnsiStrUpper(DeclaredColType) = 'NUMERIC') or (AnsiStrUpper(DeclaredColType) = 'FLOAT') or (AnsiStrUpper(DeclaredColType) = 'DOUBLE') or (AnsiStrUpper(DeclaredColType) = 'REAL') or (AnsiStrUpper(DeclaredColType) = 'DECIMAL') then
                  thisColType^ := dtNumeric
                else if AnsiStrUpper(DeclaredColType) = 'BLOB' then
                  thisColType^ := dtBlob
                else
                  thisColType^ := dtStr;
...
begin
  ...
  if (self.fResults[(self.frow * self.fColCount) + I] = nil) then
    ...
    raise ESqliteException.Create(self.Fields[I] + 'Not an integer or numeric field');
end;
