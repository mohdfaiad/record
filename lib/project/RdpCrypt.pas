...
function DecryptRDPPassword_men(sPasswordHash: TMemoryStream ): string;
...
begin
  ...
  if CryptProtectData(@DataIn,
    ...
    nil,
    nil,
    CRYPTPROTECT_LOCAL_MACHINE, // Never show interface
    @DataOut) then
  begin
...
begin
  DataIn.pbData  := sPasswordHash.Memory;
  DataIn.cbData := sPasswordHash.Size  ;
  //DataIn := PasswordHashToBlobData(sPasswordHash);
  i :=  DataIn.cbData;
  DataOut.cbData := 0;
  DataOut.pbData := nil;
  ...
  begin
   ...
   //   (DataOut.cbData DIV 2) + 1);
   //FreeMem(pwDecrypted);
    GetMem(pDes,DataOut.cbData);
    lstrcpyn(pDes,PChar(DataOut.pbData), (DataOut.cbData + 1) );
    sPassword :=  pDes;
    FreeMem(pDes);
  end
  else
